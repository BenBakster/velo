...
- **v0.3: terminal + velo-level:** реализован профиль `terminal` (X + xterm + dev, без DE, лимит 16 GiB) и CLI-переключатель профилей защиты `velo-level` (L1↔L2↔L3). Добавлены `velo-report` и `velo-egress-test`.
